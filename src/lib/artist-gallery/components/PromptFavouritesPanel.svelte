<script lang="ts">
  ...
  function moveEntry(entry: any, groupId: string) {
    promptFavourites.setEntryGroup(entry.id, groupId || null);
  }
...
<div class="flex h-full flex-col overflow-hidden bg-neutral-950 text-neutral-100">
  <div class="flex flex-wrap items-center gap-2 border-b border-neutral-800 p-2 sm:p-3">
    <input class="order-first min-w-0 basis-full rounded border border-neutral-700 bg-neutral-900 px-3 py-2 text-sm sm:flex-1 sm:basis-auto" placeholder={locale.t("prompt_favourites.search_placeholder")} bind:value={search} />
    <div class="grid w-full grid-cols-3 gap-1.5 sm:flex sm:w-auto sm:gap-2">
      <button class="min-w-0 rounded border border-indigo-600 px-2 py-2 text-xs" onclick={() => promptFavourites.addFromCurrent()}>{locale.t("prompt_favourites.save_current")}</button>
      <button class="min-w-0 rounded border border-neutral-700 px-2 py-2 text-xs" onclick={addGroup}>{locale.t("prompt_favourites.new_group")}</button>
      <button class="min-w-0 rounded border border-neutral-700 px-2 py-2 text-xs" onclick={exportLibrary}>{locale.t("prompt_favourites.export")}</button>
    </div>
  </div>
...
{#snippet Entry(entry: any)}
  <div class="flex flex-col gap-2 rounded-lg border border-neutral-800 bg-neutral-900/50 p-2 transition-colors hover:border-neutral-700 sm:flex-row sm:items-start sm:gap-1.5">
    <button
      type="button"
    ...
    <div class="grid grid-cols-[2.25rem_2.25rem_minmax(0,1fr)_2.25rem] gap-1.5 sm:flex sm:shrink-0 sm:items-center">
      <button
        type="button"
        class="flex h-9 w-9 shrink-0 items-center justify-center gap-1 rounded-md border border-neutral-700 bg-neutral-800 text-sm leading-none text-neutral-300 transition-colors hover:border-indigo-500 hover:text-indigo-200 sm:h-auto sm:w-auto sm:px-2.5 sm:py-1.5 sm:text-xs"
      ...
      <button
        type="button"
        class="flex h-9 w-9 shrink-0 items-center justify-center gap-1 rounded-md border border-neutral-700 bg-neutral-800 text-sm leading-none text-indigo-300 transition-colors hover:border-indigo-500 hover:text-indigo-200 sm:h-auto sm:w-auto sm:px-2.5 sm:py-1.5 sm:text-xs"
      ...
      <select
        aria-label={locale.t("prompt_favourites.prompt_library")}
        class="h-9 min-w-0 rounded-md border border-neutral-700 bg-neutral-800 px-2 text-xs text-neutral-300 outline-none transition-colors hover:border-indigo-500 focus:border-indigo-500 sm:h-auto sm:max-w-36 sm:px-2 sm:py-1.5"
        value={entry.groupId ?? ""}
        onchange={(e) => moveEntry(entry, e.currentTarget.value)}
      >
        <option value="">{locale.t("prompt_favourites.prompt_library")}</option>
      ...
      <button
        type="button"
        class="flex h-9 w-9 shrink-0 items-center justify-center rounded-md border border-neutral-700 bg-neutral-800 text-base leading-none text-neutral-400 transition-colors hover:border-red-500 hover:bg-red-600/10 hover:text-red-300 sm:h-auto sm:w-auto sm:px-2.5 sm:py-1.5 sm:text-xs"
